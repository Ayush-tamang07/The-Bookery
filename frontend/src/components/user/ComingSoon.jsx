...
const ComingSoon = ({ limit }) => {
  ...
  useEffect(() => {
    const fetchNewArrivals = async () => {
      try {
        const response = await apiClient.get("/book/newarrivals");
  ...
  if (loading) {
    return (
      <p className="text-center py-10 text-gray-600">Loading new arrivals...</p>
    );
  }
  ...
  if (books.length === 0) {
    return (
      <p className="text-center py-10 text-gray-500">No new arrivals found.</p>
    );
  }
